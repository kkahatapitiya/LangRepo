...
def get_model(args):
    ...
    if 'gpt' in model_name:
        ...
        return Mistral(model_name, temperature)
    elif 'mixtral' in model_name.lower():
        return Mixtral(model_name, temperature)
...
class GPT(Model):
    ...
    def forward(self, head, prompts, max_new_tokens=20, single_turn=True,):
        del single_turn, max_new_tokens
        messages = [
...
class LLaMA(Model):
    ...
    def forward(self, head, prompts, max_new_tokens=20):
        del head
        prompt = prompts[0]
...
class Mistral(Model):
    ...
    def forward(self, head, prompts, max_new_tokens):
        del head
        prompt = prompts[0]
        sequences = self.pipeline(
            prompt,
...
class Mixtral(Model):
    ...
    def forward(self, head, prompts, max_new_tokens, single_turn=True):
        del head, single_turn
        prompt = prompts[0]
        sequences = self.pipeline(
            prompt,
            do_sample=False,
